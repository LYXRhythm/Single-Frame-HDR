...
def test(model, test_dataloader, save_path, best_psnr=None, save_img=False):
    ...
    for i, batch in enumerate(test_dataloader):
        ...
        # fakes_np = fakes.cpu().numpy()
        # targets_org_np = targets_org.cpu().numpy()
        ...
        if save_img:
            ...
            save_file_path = join(save_path, f"{name}.jpg")
    ...
    avg_psnr_normalized /= len(test_dataloader)
    ...
    return avg_psnr_out, test_cost,avg_psnr_normalized
...
if __name__ == "__main__":
    ...
    ckpt = torch.load(join(hparams.save_models_root, f"model{hparams.epoch:0>4}.pth"),map_location=torch.device('cpu'))
    ...
    print("\rTest PSNR-L results: {:>0.2f}dB; Test cost: {:0>5.2f}s;Test PSNR-u results: {:>0.2f}dB;".format(*test(model, test_dataloader, hparams.save_images_root, save_img=True)))